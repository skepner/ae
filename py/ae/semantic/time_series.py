...
def style(chart: ae_backend.chart_v3.Chart, time_series: TimeSeriesRange, name: str = "ts-", title_prefix: str = None, title_style: dict[str, object] = {}, priority: int = 5000, front_priority: int = 500):
    ...
    for dfirst, dlast in time_series.range_begin_end():
        ...
        front_style.add(chart=chart, style_name=sname[1:], references=["-reset", "-continent", "-ts-old-new", sname, "-vaccines"], title=title, title_style=title_style, show_legend=True, legend_counter=True, style_priority=front_priority + priority_inc)
        priority_inc += 1
