...
namespace ae::chart::v2
{
    ...
    class LayoutRandomizer
    {
     public:
        ...
        virtual ae::draw::v1::PointCoordinates get(number_of_dimensions_t number_of_dimensions)
            {
                ae::draw::v1::PointCoordinates result(number_of_dimensions);
...
// ----------------------------------------------------------------------
    ...
    {
     public:
        LayoutRandomizerWithLineBorder(double diameter, const ae::draw::v1::LineSide& line_side, seed_t seed = std::nullopt)
            : LayoutRandomizerPlain(diameter, seed), line_side_(line_side) {}

        ae::draw::v1::PointCoordinates get(number_of_dimensions_t number_of_dimensions) override { return line().fix(LayoutRandomizerPlain::get(number_of_dimensions)); }

        ae::draw::v1::LineSide& line() { return line_side_; }
        const ae::draw::v1::LineSide& line() const { return line_side_; }

     protected:
        using LayoutRandomizerPlain::get;

     private:
        ae::draw::v1::LineSide line_side_;
...
// ----------------------------------------------------------------------
    ...
    std::shared_ptr<LayoutRandomizer> randomizer_border_with_current_layout_area(const ProjectionModify& projection, double diameter_multiplier, const ae::draw::v1::LineSide& line_side, LayoutRandomizer::seed_t seed = std::nullopt);
